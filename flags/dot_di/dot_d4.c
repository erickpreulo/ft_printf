...
/*   Updated: 2021/04/15 21:51:33 by egomes           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	dot_d4_1(t_dot *dotd, t_obj *obj, const char *str)
{
	if (dotd->buff[-1] >= '0' && dotd->buff[-1] <= '9')
	{
		...
		if ((dotd->a >= dotd->b || dotd->cpya > dotd->b) && (dotd->a > dotd->l || dotd->cpya > dotd->l))
		{
			if (dotd->cpya > 0)
		...
		else if ((dotd->b > dotd->a || dotd->b > dotd->cpya) && (dotd->a > dotd->l || dotd->cpya > dotd->l))
		{
			if (dotd->cpya > 0)
				ft_memcpy(dotd->af, dotd->s, dotd->cpya - dotd->l, dotd->l);
			else
...
void	dot_d4(t_dot *dotd, t_obj *obj, const char *str)
{
	...
	if ((dotd->a > dotd->l || dotd->cpya > dotd->l) && dotd->buff[-1] <= '0' && dotd->buff[-1] >= '9')
	{
		if (dotd->cpya > 0)
			ft_memcpy(dotd->af, dotd->s, dotd->cpya - dotd->l, dotd->l);
		else
	...
	else if (dotd->buff[-1] <= '0' && dotd->buff[-1] >= '9')
		ft_putchars(dotd->s, obj);
}
